...
mbtchar_chmod(char *name, mode_t mode)
{
	...
	if (inode->i_op->setattr)
		...
		ret = inode->i_op->setattr(mnt_idmap(path.mnt), path.dentry,
					   &newattrs);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 12, 0)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 2, 16)
		ret = simple_setattr(mnt_idmap(path.mnt), path.dentry,
				     &newattrs);
#else
...
chardev_read(struct file *filp, char *buf, size_t count, loff_t * f_pos)
{
	...
	PRINTM(DATA,
	       "BT: chardev_read count=%d pkt_type: 0x%x, len=%d skb=%p\n",
...
chardev_open(struct inode *inode, struct file *filp)
{
	...
	if (m_dev->dev_type == BT_TYPE && m_dev->notify) {
...
chardev_release(struct inode *inode, struct file *filp)
{
	...
	if (m_dev != NULL && m_dev->dev_type == BT_TYPE && m_dev->notify) {
		PRINTM(CMD,
